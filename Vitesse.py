...
Files_reduced=['001_ChHo-001.xlsx','002_BaEl-001.xlsx','011_YaJu-001.xlsx','016_LaCh-001.xlsx','022_DrGe-001.xlsx','021_GuGi-001.xlsx']
for i in range (len(Files_reduced)) :
    File=Files_reduced[i]
    vitesse=pd.read_excel(File, sheet_name ="Segment Velocity")
    vitesse.drop( vitesse[ (vitesse['Frame'] > 400) ].index, inplace=True)
    Frame=vitesse.loc[:,'Frame']
    ...
    for  k in range(len(V1)) :
        V.append (np.sqrt (V1[k] * V1[k] + V2[k] *V2[k] + V3[k] *V3[k]))
    plt.plot(Frame,V)
plt.legend(['25-1','22-0','35-0','35-1','58-1','68-0']) 
...
for i in range (len(Files_reduced)) :
    ...
    V1=vitesse.loc[:,vitesse.columns[13]] #Acceleration of T8
    V2=vitesse.loc[:,vitesse.columns[14]]
    V3=vitesse.loc[:,vitesse.columns[15]]
